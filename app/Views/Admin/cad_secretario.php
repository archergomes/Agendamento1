...
<head>
    ...
    <title>Cadastrar Secretário - Administrador - Centro de Saúde Da Matola II</title>
    <meta name="description" content="Cadastrar ou editar secretários no Centro de Saúde Da Matola II">
...
<body>
        ...
        <main class="main-content">
            <div class="container mx-auto px-4 py-8">
                <div class="text-center mb-6">
                    <h2 class="form-title" id="form-title">Cadastrar Novo Secretário</h2>
                    <p class="form-subtitle">Preencha os dados abaixo para cadastrar ou editar um secretário.</p>
                </div>

                <!-- Secretary Form -->
                <div class="form-container">
                    <div class="card-header">
                        <h3><i class="fas fa-user-tie text-blue-500 mr-2"></i>Dados do Secretário</h3>
                    </div>
                    <form id="secretary-form" onsubmit="saveSecretary(event)">
                        <div class="form-grid">
                            <div class="form-group form-full-width">
                                <label for="secretary-name">Nome Completo <span class="required">*</span></label>
                                <input type="text" id="secretary-name" class="form-input" required placeholder="Nome completo">
                                <div class="form-error" id="name-error">Por favor, insira um nome válido</div>
                            </div>

                            <div class="form-group">
                                <label for="secretary-phone">Telefone <span class="required">*</span></label>
                                <input type="tel" id="secretary-phone" class="form-input" required placeholder="+258 84 1234567">
                            ...
                            <div class="form-group">
                                <label for="secretary-bi">BI <span class="required">*</span></label>
                                <input type="text" id="secretary-bi" class="form-input" required placeholder="Número do BI">
                                <div class="form-error" id="bi-error">Por favor, insira um BI válido</div>
                            </div>

                            <div class="form-group">
                                <label for="secretary-email">Email <span class="required">*</span></label>
                                <input type="email" id="secretary-email" class="form-input" required placeholder="exemplo@email.com">
                            ...
                            <div class="form-group form-full-width" id="password-field">
                                <label for="secretary-password">Senha <span class="required" id="password-required">*</span></label>
                                <input type="password" id="secretary-password" class="form-input" placeholder="Mínimo 6 caracteres">
                                <div class="form-hint">Mínimo 6 caracteres (obrigatório para novos secretários)</div>
    ...
    <script>
        ...
        // ==================== FETCH SECRETARY ====================
        async function fetchSecretary(id) {
            try {
                ...
                const response = await fetch('<?= site_url('admin/get_secretary_details') ?>', {
                    method: 'POST',
        ...
        function validateForm(isEditMode) {
            ...
            // Password validation (only for new secretaries)
            if (!isEditMode) {
        ...
        async function saveSecretary(event) {
            ...
            const endpoint = isEditMode ? '<?= site_url('admin/update_secretary') ?>' : '<?= site_url('admin/create_secretary') ?>';
            ...
            try {
                ...
                if (!response.ok) {
                    showNotification(result.error || 'Erro ao salvar secretário.', 'error');
                    return;
                }
                ...
                if (result.error) {
                    ...
                    showNotification(result.success || (isEditMode ? 'Secretário atualizado com sucesso!' : 'Secretário cadastrado com sucesso!'), 'success');
                    setTimeout(() => {
                        window.location.href = '<?= site_url('admin/secretarios') ?>';
                    }, 1500);
                }
            } catch (error) {
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            const inputs = ['secretary-name', 'secretary-bi', 'secretary-email', 'secretary-password'];
            inputs.forEach(id => {
                ...
                if (input) {
                    ...
                    input.addEventListener('blur', function() {
                        if (this.value.trim() && this.id !== 'secretary-password') {
                            this.classList.add('input-success');
                        }
                    });
                }
            ...
            if (bi) {
                ...
                document.getElementById('save-btn').innerHTML = '<i class="fas fa-save mr-1"></i>Atualizar';
                
                const secretary = await fetchSecretary(bi);
                if (secretary) {
                    document.getElementById('secretary-name').value = secretary.name || '';
                    document.getElementById('secretary-phone').value = secretary.phone || '';
                    document.getElementById('secretary-bi').value = secretary.bi || '';
                    ...
                    if (secretary.phone && validateMozambicanPhone(secretary.phone)) {
                        document.getElementById('secretary-phone').classList.add('input-success');
                    }
                } else {
                    showNotification('Secretário não encontrado.', 'error');
                }
            } else {
                formTitle.textContent = 'Cadastrar Novo Secretário';
                biInput.removeAttribute('readonly');
                document.getElementById('save-btn').innerHTML = '<i class="fas fa-save mr-1"></i>Salvar';
