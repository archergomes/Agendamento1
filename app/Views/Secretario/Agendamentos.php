...
<head>
    ...
    <style>
        ...
        .btn-view { background-color: #e5e7eb; color: #374151; }
        ...
        .patient-suggestions {
            ...
            margin-top: 0.25rem; max-height: 190px; overflow-y: auto;
            box-shadow: 0 10px 25px rgba(0,0,0,0.12); z-index: 20;
        }
        .patient-suggestions.show { display: block; }
        .patient-suggestion-item { padding: 0.55rem 0.8rem; cursor: pointer; font-size: 0.85rem; border-bottom: 1px solid #f3f4f6; }
        .patient-suggestion-item:last-child { border-bottom: none; }
        .patient-suggestion-item:hover { background-color: #f3f5f9; }
        .patient-suggestion-item .phone { font-size: 0.72rem; color: #9ca3af; }
        .patient-selected-chip {
            display: none; align-items: center; gap: 0.5rem;
            background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 0.5rem;
            padding: 0.5rem 0.75rem; font-size: 0.82rem; color: #1d4ed8; margin-top: 0.4rem;
        }
        .patient-selected-chip.show { display: flex; }
        .patient-selected-chip button { margin-left: auto; background: none; border: none; color: #1d4ed8; cursor: pointer; }

        /* Details modal */
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.85rem 1.5rem; margin-bottom: 1rem; }
        .info-grid .label { font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.03em; color: var(--ink-500); font-weight: 600; margin-bottom: 0.15rem; }
        .info-grid .value { font-size: 0.9rem; color: var(--ink-900); font-weight: 500; }
        .info-note { background: var(--paper); border-radius: 0.5rem; padding: 0.7rem 0.85rem; font-size: 0.83rem; color: var(--ink-700); margin-top: 0.5rem; }
        .details-actions { display: flex; gap: 0.5rem; flex-wrap: wrap; border-top: 2px solid #f3f4f6; padding-top: 1rem; margin-top: 1rem; }
        .details-actions .btn { flex: 1; min-width: 120px; justify-content: center; }
        ...
        @media (max-width: 640px) {
            ...
            .filter-btn { font-size: 0.65rem; padding: 0.25rem 0.6rem; }
...
<body>
    ...
    <script>
        ...
        // ==================== DADOS DE APOIO ====================
        // TODO API: substituir estes três blocos por:
        //   GET /secretario/get_specialties
        //   GET /secretario/get_doctors?specialty_id=
        //   GET /secretario/search_patients?q=
        // Mantidos como dados fixos apenas para a interface funcionar
        // enquanto os endpoints não existem no backend.
        const SPECIALTIES = [
        ...
        const DOCTORS = [
            { id: 1, nome: 'Dr. Armando Cossa', especialidade_id: 1 },
            { id: 2, nome: 'Dra. Fátima Nhaca', especialidade_id: 2 },
            { id: 3, nome: 'Dr. Bento Machel', especialidade_id: 4 },
            { id: 4, nome: 'Dra. Ivone Sitoe', especialidade_id: 3 },
            { id: 5, nome: 'Dr. Paulo Zunguza', especialidade_id: 5 },
            { id: 6, nome: 'Dra. Amélia Tembe', especialidade_id: 1 },
        ];
        const PATIENTS = [
            { id: 101, nome: 'Ana Muchanga', telefone: '84 123 4567' },
            { id: 102, nome: 'Carlos Nhantumbo', telefone: '82 234 5678' },
            { id: 103, nome: 'Beatriz Cumbe', telefone: '87 345 6789' },
            { id: 104, nome: 'Eduardo Massingue', telefone: '84 456 7890' },
            { id: 105, nome: 'Ana Paula Sitoe', telefone: '86 567 8901' },
        ];

        // Guarda a última lista carregada, usada por detalhes/editar/exportar
        ...
        function editAppointment(id) {
            ...
            if (!appt) { showNotification('Agendamento não encontrado na lista actual.', 'error'); return; }
            closeModal('details-modal');
        ...
        patientSearchInput.addEventListener('input', debounce(function (e) {
            ...
            // TODO API: GET /secretario/search_patients?q=... — usando lista fixa por agora
        ...
        async function submitAppointmentForm(event) {
            ...
            if (!patientId) { showNotification('Seleccione um paciente da lista de sugestões.', 'error'); return; }
            if (!specialtyId || !doctorId) { showNotification('Seleccione a especialidade e o médico.', 'error'); return; }
            if (!date || !time) { showNotification('Indique a data e a hora do agendamento.', 'error'); return; }
            ...
            // TODO API: criar → POST /secretario/create_appointment
            //           editar → POST /secretario/update_appointment
        ...
        function viewAppointment(id) {
            // TODO API: idealmente GET /secretario/get_appointment/{id} para incluir
            // telefone/observações que a listagem paginada pode não trazer.
            const appt = lastAppointments.find(a => String(a.ID_Agendamento) === String(id));
            if (!appt) { showNotification('Agendamento não encontrado.', 'error'); return; }
        ...
        // ==================== EXPORTAR / IMPRIMIR ====================
        function exportCSV() {
            if (!lastAppointments.length) { showNotification('Não há dados para exportar.', 'warning'); return; }
        ...
        function exportPrint() {
            if (!lastAppointments.length) { showNotification('Não há dados para imprimir.', 'warning'); return; }
            const rows = lastAppointments.map(a => `
                <tr>
        ...
        async function loadAppointments(page = 1) {
            ...
            try {
                ...
                list.innerHTML = lastAppointments.map(appt => {
                    const statusClass = (appt.Status || 'pendente').toLowerCase();
                    const statusIcons = {
                        'pendente': 'fa-clock', 'confirmado': 'fa-check-circle',
                        'concluido': 'fa-check-double', 'cancelado': 'fa-times-circle'
                    };
        ...
        function renderPagination() {
            ...
            prevBtn.addEventListener('click', () => { if (currentPage > 1) loadAppointments(currentPage - 1); });
            ...
            nextBtn.addEventListener('click', () => { if (currentPage < totalPages) loadAppointments(currentPage + 1); });
